...
| ----------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Structural Validation**     | Completeness, Validity, Uniqueness, Integrity | Enforces structure and formatting rules using:<br>• **Data Type Conformance** — Match types (e.g., int, string, timestamp)<br>• **Null Checks** — Required fields must not be null<br>• **Uniqueness / PK Checks** — Prevent duplicate key entries<br>• **Referential Integrity** — Validate FK relationships (e.g., `customer_id` exists)<br>• **Format Validation** — Check specific patterns:<br> – Dates (`YYYY-MM-DD`, ISO 8601)<br> – Currency (`$99.99`, `€1.000,00`)<br> – Time zones (`+05:30`, `UTC`)<br>🛠️ RDBMS supports these natively; for data lakes or NoSQL, use ETL/dbt/GE/Deequ. |
| **Business Rule Validation**  | Consistency                                   | Business/domain rules to catch cross-field inconsistencies:<br>• `order_amount > 0`<br>• `status IN ('SHIPPED', 'CANCELLED')`<br>• `start_date < end_date`<br>• Revenue drop shouldn't exceed 20%<br>Implemented via SQL, Python, dbt tests, or Great Expectations.                                                                                                                                                                                                                                                                                                                                  |
...
| **Time-Series Consistency**   | Completeness, Accuracy                        | Ensures time-ordered data is logically consistent:<br>• No missing intervals<br>• No duplicate timestamps<br>• Events are in correct order<br>Key for streaming, IoT, finance, and log systems.                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Data Lineage & Provenance** | Integrity, Traceability                       | Tracks origin, movement, and transformation of data:<br>• Where it came from<br>• How it was processed<br>• Who owns or modified it<br>Enables governance, compliance, and root-cause analysis.                                                                                                                                                                                                                                                                  
